...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        self.bounds = CGRectMake(0, 0, 414, 50);
    }
    return self;
}
...
- (void)awakeFromNib {
    ...
    self.logButton.layer.cornerRadius = self.logButton.bounds.size.height / 2.0;
...
- (IBAction)onLogButtonClicked:(id)sender {
    if ([self.delegate respondsToSelector:@selector(onLogButtonClicked:)]) {
        [self.delegate onLogButtonClicked:sender];
    }
}
