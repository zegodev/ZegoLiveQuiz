//
//  ZegoSettingTableViewController.m
//
//
...
@interface ZegoSettingTableViewController () <UITextFieldDelegate, MFMailComposeViewControllerDelegate>
...
@implementation ZegoSettingTableViewController
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    {
        [ZegoSetting sharedInstance].userID = self.userID.text;
        [ZegoSetting sharedInstance].userName = self.userName.text;
    }
...
- (IBAction)toggleTestEnv:(id)sender {
    ...
    [ZegoSetting sharedInstance].useTestEnv = s.on;;
}
...
- (void)keyboardWillHide:(NSNotification *)notification {
    if ([ZegoSetting sharedInstance].appType == ZegoAppTypeCustom) {
        if (self.appIDText.text.length == 0 && self.appSignText.text.length == 0) { // 自定义版本下不输入 AppID 和 AppSign，自动切换为国内版
            [ZegoSetting sharedInstance].appType = ZegoAppTypeUDP;
            ...
            [[ZegoSetting sharedInstance] setCustomAppID:appID sign:self.appSignText.text];
        }
        ...
        if ([self.appIDText.text isEqualToString:@"1"]) {    // 当用户选择自定义，并且输入的 AppID 为 1 时，自动识别为 RTMP 版本且填充 AppSign
            ...
            [[ZegoSetting sharedInstance] setCustomAppID:1 sign:signkey];
...
- (IBAction)onClose:(id)sender {
    // Warning: release then init it, AppSupport updateRoomList will take effect
    [ZegoSDKManager releaseApi];
    [ZegoSDKManager api];
...
- (void)testPullStream {
    ...
    UIAlertAction *confirm = [UIAlertAction actionWithTitle:NSLocalizedString(@"确定",nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ZegoTestPullViewController *testViewController = [[ZegoTestPullViewController alloc] initWithNibName:@"ZegoTestPullViewController" bundle:nil];
...
- (void)testPushStream {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"请选择推流模式",nil)
                                                                             message:nil
                                                                      preferredStyle:UIAlertControllerStyleActionSheet];

    UIAlertAction *singleAnchorMode  = [UIAlertAction actionWithTitle:NSLocalizedString(@"单主播模式", nil)
                                                                style:UIAlertActionStyleDefault
                                                              handler:^(UIAlertAction * action) {
                                                                  [self pushStreamWithFlag:ZEGOAPI_SINGLE_ANCHOR];
                                                              }];

    UIAlertAction *multiAnchorMode  = [UIAlertAction actionWithTitle:NSLocalizedString(@"连麦模式", nil)
                                                               style:UIAlertActionStyleDefault
                                                             handler:^(UIAlertAction * action) {
                                                                 [self pushStreamWithFlag:ZEGOAPI_JOIN_PUBLISH];
                                                             }];

    UIAlertAction *mixStreamMode  = [UIAlertAction actionWithTitle:NSLocalizedString(@"混流模式", nil)
                                                             style:UIAlertActionStyleDefault
                                                           handler:^(UIAlertAction * action) {
...
- (void)pushStreamWithFlag:(enum ZegoAPIPublishFlag)flag {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"指定ID推流",nil)
                                                                             message:nil
    ...
    UIAlertAction *confirm = [UIAlertAction actionWithTitle:NSLocalizedString(@"确定",nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ZegoTestPushViewController *testViewController = [[ZegoTestPushViewController alloc] initWithNibName:@"ZegoTestPushViewController" bundle:nil];
...
- (void)loadEnvironmentSettings {
    self.testEnvSwitch.on = [ZegoSetting sharedInstance].useTestEnv;
    [self.appTypePicker selectRow:[ZegoSetting sharedInstance].appType inComponent:0 animated:NO];
    
    [self loadAppID];
}

- (void)loadAppID {
    ZegoAppType type = [ZegoSetting sharedInstance].appType;
    
    // 导航栏标题随设置变化
    NSString *title = [NSString stringWithFormat:@"ZEGO(%@)", [ZegoSetting sharedInstance].appTypeList[type]];
    self.tabBarController.navigationItem.title =  NSLocalizedString(title, nil);
    
    // 自定义的 APPID 来源于用户输入
    uint32_t appID = [ZegoSetting sharedInstance].appID;
    NSData *appSign = [ZegoSetting sharedInstance].appSign;
    if (type == ZegoAppTypeCustom) {
        NSString *appSignString = [ZegoSetting sharedInstance].customAppSign;
...
- (void)loadAccountSettings {
    self.userID.text = [ZegoSetting sharedInstance].userID;
    self.userName.text = [ZegoSetting sharedInstance].userName;
}

- (void)loadVideoSettings {
    self.version.text = [ZegoLiveRoomApi version];
    [self.presetPicker selectRow:[ZegoSetting sharedInstance].presetAVConfigIndex inComponent:0 animated:YES];
...
- (void)updateVideoSettingUI {
    ...
    CGSize r = [ZegoSetting sharedInstance].avConfig.videoEncodeResolution;
...
- (void)sendEmail {
    if ([MFMailComposeViewController canSendMail]) {
        ...
        NSString *subject = [NSString stringWithFormat:@"%d-%@-%@", [ZegoSetting sharedInstance].appID, [ZegoSetting sharedInstance].userID, dateString];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (pickerView == self.presetPicker) {
        return [ZegoSetting sharedInstance].presetVideoQualityList.count;
    } else if (pickerView == self.appTypePicker) {
        return [ZegoSetting sharedInstance].appTypeList.count;
    } else {
        return 0;
    }
}
...
{
    if (pickerView == self.presetPicker) {  // 视频质量 picker
        if (row >= [ZegoSetting sharedInstance].presetVideoQualityList.count) {
            return;
        }
        
        NSLog(@"%s: %@", __func__, [ZegoSetting sharedInstance].presetVideoQualityList[row]);
        
        [[ZegoSetting sharedInstance] selectPresetQuality:row];
        
        [self updateVideoSettingUI];
        
    } else if (pickerView == self.appTypePicker) {  // App版本 picker
        if (row >= [ZegoSetting sharedInstance].appTypeList.count) {
            return;
        }
        
        NSLog(@"%s: %@", __func__, [ZegoSetting sharedInstance].appTypeList[row]);
        
        [ZegoSetting sharedInstance].appType = (ZegoAppType)row;
        [self loadAppID];
    }
...
- (NSString*)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component
{
    if (pickerView == self.presetPicker)
    {
        ...
        return [[ZegoSetting sharedInstance].presetVideoQualityList objectAtIndex:row];
    } else if (pickerView == self.appTypePicker) {
        if (row >= [ZegoSetting sharedInstance].appTypeList.count) {
            return @"ERROR";
        }
        
        return [[ZegoSetting sharedInstance].appTypeList objectAtIndex:row];
    }
    return nil;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    } else if (indexPath.section == 4 && indexPath.row == 0) {  // 高级选项，不同 App 设置不同
        
#ifdef VIDEOLIVE
       [self performSegueWithIdentifier:@"ShowVideoLiveAdvanced" sender:nil];
#endif
        
#ifdef VIDEOTALK
       [self performSegueWithIdentifier:@"ShowVideoTalkAdvanced" sender:nil];
#endif
